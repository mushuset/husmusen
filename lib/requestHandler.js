...
export default function requestHandler(req, res, next) {
    ...
    getRequestData(req).then(
        data => {
            ...
            const inputFormatter = inputFormatHandlers[inputFormat] ?? inputFormatHandlers["application/json"]
            const inputData = inputFormatter(data || defaultData[inputFormat])
            req.data = inputData

            // Set the input data at `req.data`
            const outputFormatter = outputFormatHandlers[inputFormat] ?? outputFormatHandlers["application/json"]
            res.sendit = outputData => res.header("Content-Type", outputFormat).send(outputFormatter(outputData))
